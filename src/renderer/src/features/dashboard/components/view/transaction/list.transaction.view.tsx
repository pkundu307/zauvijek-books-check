...
import { SimpleTable } from '@renderer/components/table/simple_table'
...
export default function ListTransactionView() {
  const columns = [
    {
      Header: 'Date',
      accessor: (row: transactionType) => <Text fontSize="sm">{parseDate(row.created_at)}</Text>,
      width: getInnerWidth(20)
    },
    {
      Header: 'Transaction Type',
      accessor: (row: transactionType) => <Text fontSize="sm">{row.transaction_type}</Text>,
      width: getInnerWidth(20)
    },
    {
      Header: 'Payment Type',
      accessor: (row: transactionType) => <Text fontSize="sm">{row.payment_type}</Text>,
      width: getInnerWidth(20)
    },
    {
