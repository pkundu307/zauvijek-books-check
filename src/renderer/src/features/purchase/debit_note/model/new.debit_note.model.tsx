...
import NewDebitNoteController from '../controller/new.debit_note.controller'
...
export default function NewDebitNoteModel() {
  /**
   * ----------------------------------------------------------------------
   *  LIBRARY HOOKS START
   *
  ...
  const newDebitNote = useMutation({
    mutationFn: createPurchase,
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ['getPurchases', 'debit_note']
      })
      toast({
  ...
  function handleNew(values: any) {
    ...
    newDebitNote?.mutate(updateValues)
  }
  ...
  if (newDebitNote?.isPending) {
    return <Loading />
  }
  return <NewDebitNoteController handleNew={handleNew} />
}
