...
export default function DownshiftSearchParty(props: any) {
  const { mode20, mode30, mode60 } = useThemeMode()
          ...
          itemList.map((item: any, index: number) => (
            ...
            >
              <Flex justify="space-between">
                <Text fontSize={props.size}>{item.party_name}</Text>
                <Text fontSize={props.size} color={mode60}>
                  {parseCurrency(item.closing_balance || 0)}
                </Text>
              </Flex>
            </ListItem>
          ))}
...
const getFilter = (inputValue: any) => {
  return function itemsFilter(item: any) {
    return !inputValue || item.party_name.toLowerCase().includes(inputValue)
  }
}
