...
export default class PartyActivityService extends Database {
  ...
  async createPartyActivity(params: any): Promise<PartyActivity[]> {
    try {
      const partyActivityRepository =
        this.dataSource.getRepository(PartyActivity);
      const newPartyActivity = partyActivityRepository.create(params);
      const savedPartyActivity =
        await partyActivityRepository.save(newPartyActivity);
  ...
  public async getAll(): Promise<PartyActivity[]> {
    const partyActivityRepository =
      this.dataSource.getRepository(PartyActivity);
    const queryBuilder =
      partyActivityRepository.createQueryBuilder("partyActivity");
  ...
  async updatePartyActivity(
    id: string,
    updates: Partial<PartyActivity>
  ): Promise<void> {
    try {
      const partyActivityRepository =
        this.dataSource.getRepository(PartyActivity);
