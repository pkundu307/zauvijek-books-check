...
console.log(params.phone_no);
    ...
    const boldListItems = ['BILL T', 'ADI', `Mobile: ${params.phone_no}`]
    ...
    // Reset font to regular
    doc.font('Helvetica')
    const tableData = ['21', '04/09/2024', '11/9/24']
  ...
  async generateInvoiceTable(doc): Promise<void> {
    ...
    const tableData = ['S No.', 'ITEMS', 'QTY', 'DATE', 'AMOUNT']
    let x = 30;
    ...
    doc.fontSize(15)
    .text("Amount Chargeable (in words)",11,445)

    doc.rect(10, 459, 570, 30).stroke()
    doc.fillColor('black').fontSize(12).text("HSN/SAC",70,466)
